...
const IdeaHub = () => {
  ...
  const validateForm = () => {
    ...
    if (!/^[a-zA-Z0-9]{1,6}$/.test(formData.userEPF)) {
      formErrors.userEPF =
        "User EPF should only contain letters and numbers, and not be longer than 6 characters.";
    }

    if (!/^[a-zA-Z\s]{1,120}$/.test(formData.deptOrBranch)) {
      formErrors.deptOrBranch =
        "Department/Branch should only contain letters and be up to 120 characters.";
    }

    if (!/^[a-zA-Z\s]{1,120}$/.test(formData.name)) {
      formErrors.name =
        "Name of the User should only contain letters and be up to 120 characters.";
    }

    if (!/^[a-zA-Z\s]{1,120}$/.test(formData.userIdea)) {
      formErrors.userIdea =
        "User Idea should only contain letters and be up to 120 characters.";
    }
          ...
          <div style={formContainerStyle}>
            <form onSubmit={handleSubmit}>
              ...
              <div style={formRowStyle}>
                <div style={formColumnStyle}>
                  ...
                  <input
                    type="text"
                    name="userEPF"
                    placeholder="eg: L1234"
                    style={inputStyle}
                    value={formData.userEPF}
                    onChange={handleChange}
                  />
                  {errors.userEPF && (
                    <p style={{ color: "red" }}>{errors.userEPF}</p>
                  )}
                </div>
                <div style={formColumnStyle}>
                  ...
                  <input
                    ...
                    style={inputStyle}
                    value={formData.deptOrBranch}
                    onChange={handleChange}
                  />
                  {errors.deptOrBranch && (
                    <p style={{ color: "red" }}>{errors.deptOrBranch}</p>
                  )}
                </div>
              </div>
              <div style={formColumnStyle}>
              ...
              <div style={formColumnStyle}>
                ...
                />
                {errors.userIdea && (
                  <p style={{ color: "red" }}>{errors.userIdea}</p>
                )}
              </div>
              <br />
